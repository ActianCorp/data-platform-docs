...
Only users in the useradmin and admin groups have the privilege to maintain other users. For more information, see [User Groups](../DBaaS_User/User_Groups.md).
...
1. [Launch Query Editor](../DBaaS_User/Launch_Query_Editor.md).
...
All of the following examples can be run in Query Editor or from the command line interface (the [Actian SQL CLI](../SQLLanguage/Actian_SQL_CLI.md), installed with the Avalanche Client Runtime Package).

!!! note "Note"

    All examples assume connection to the User Management database (iidbdb). See examples of connections in [Overview of User Management](../DBaaS_User/Overview_of_User_Management.md).
...
Log in to Actian as the user dbuser. Note that demo_table is an example table and is not provided by default.
