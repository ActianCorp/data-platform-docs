...
!!! note "Note"
    ...
    This section applies only to Azure.

The dbuser is the local database user with administrative privileges on the db database. It is able to manage other users (add, alter, remove; see [Manage Users](../DBaaS_User/Manage_Users_2.md)) and has all privileges for sample data (select, alter, delete, copy, reference, and so on). It is a member of the dbadmingrp (see [User Groups](../DBaaS_User/User_Groups.md)).
