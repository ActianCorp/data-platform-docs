...
| Encryption Key | Displays the method of data at rest encryption:•No Encryption Key – Warehouse was created before data encryption was implemented in Actian. Encryption can only be added at warehouse creation and not after a warehouse has been created.•Actiand-Managed Encryption Key – Warehouse was created before an external customer-managed KMS key alias was created, or Actiand-managed encryption key was set as the default KMS. For more information, see [Using an Actian-managed Encryption Key](../User/Using_an_Actian-managed_Encryption_Key.md).•<Key alias> – Warehouse was created after an external customer-managed KMS key alias was created and assigned as the default KMS.•Note: Warehouse encryption is not currently supported for Google Cloud warehouses. |
