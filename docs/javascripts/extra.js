...
// Compatibility shim: `document$` / `location$` are Material for MkDocs RxJS
// observables. Zensical's modern theme does not expose them, so guard every
// use. `onDocument()` runs `fn` on each (instant) navigation when the
// observable exists, and falls back to a single DOMContentLoaded run otherwise.
...
function onLocation(fn) {
    if (window.location$ && typeof window.location$.subscribe === "function") {
        window.location$.subscribe(fn);
    }
}

// NOTE: The "View as Markdown" link is no longer injected here. It is now
// rendered as a native content-action button (the file-code icon at the top
// right of the content) via the `content` block override in
// theme_overrides/main.html, matching the default Zensical layout.
