...
| programquit | Integer | Returns 1 if the programquit option is enabled (using SET_SQL(PROGRAMQUIT). If programquit is enabled, the following errors cause embedded SQL applications to terminate:<br>• Issuing a query when not connected to a database<br>• Failure of Actian Data Platform<br>• Failure of communications services<br>• Returns 0 if applications continue after encountering such errors. |
